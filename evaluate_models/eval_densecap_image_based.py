...
class DenseCaptioningEvaluator:
    ...
    def evaluate_captions(self, img_ids):
        ...
        # text scores
        if not os.path.exists('../dataset/densecap_i_scores_spice.csv'):
            ...
            for i, s in zip(img_ids, scores):
                values[i] = s
            pd.DataFrame().from_dict(values, orient='index').to_csv('../dataset/densecap_i_scores_spice.csv')
        if not os.path.exists('../dataset/densecap_i_scores_rouge.csv'):
            ...
            for i, s in zip(img_ids, scores):
                values[i] = s
            pd.DataFrame().from_dict(values, orient='index').to_csv('../dataset/densecap_i_scores_rouge.csv')
        if not os.path.exists('../dataset/densecap_i_scores_meteor.csv'):
            ...
            for i, s in zip(img_ids, scores):
                values[i] = s
            pd.DataFrame().from_dict(values, orient='index').to_csv('../dataset/densecap_i_scores_meteor.csv')
        if not os.path.exists('../dataset/densecap_i_scores_cider.csv'):
            ...
            for i, s in zip(img_ids, scores):
                values[i] = s
            pd.DataFrame().from_dict(values, orient='index').to_csv('../dataset/densecap_i_scores_cider.csv')
        if not os.path.exists('../dataset/densecap_i_scores_bleu_1.csv'):
            ...
            for i, s in zip(img_ids, scores_1):
                values_1[i] = s
            pd.DataFrame().from_dict(values_1, orient='index').to_csv('../dataset/densecap_i_scores_bleu_1.csv')
            scores_2 = [np.average(s) for s in scores_2]
            values_2 = dict()
            for i, s in zip(img_ids, scores_2):
                values_2[i] = s
            pd.DataFrame().from_dict(values_2, orient='index').to_csv('../dataset/densecap_i_scores_bleu_2.csv')
            scores_3 = [np.average(s) for s in scores_3]
            values_3 = dict()
            for i, s in zip(img_ids, scores_3):
                values_3[i] = s
            pd.DataFrame().from_dict(values_3, orient='index').to_csv('../dataset/densecap_i_scores_bleu_3.csv')
            scores_4 = [np.average(s) for s in scores_4]
            values_4 = dict()
            for i, s in zip(img_ids, scores_4):
                values_4[i] = s
            pd.DataFrame().from_dict(values_4, orient='index').to_csv('../dataset/densecap_i_scores_bleu_4.csv')
